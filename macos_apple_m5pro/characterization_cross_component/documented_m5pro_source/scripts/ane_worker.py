...
def main():
    ...
    input_data = np.random.default_rng(20260713).random((1, 3, 224, 224), dtype=np.float32)
    ...
    with counter_path.open("r+b") as handle:
        ...
        print(f"ANE_READY model={args.model} target_rate={args.target_rate}", flush=True)
        count = 0
