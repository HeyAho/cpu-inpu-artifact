...
#define NPU_SERVER \
    "external/experiments_organized/" \
    "13_flush_reload_timing_heavy_m4_20260715/bin/" \
    "npu_flush_server_objc"

#define DEFAULT_MODEL \
    "external/flush_reload_poc/" \
...
static npu_t npu_start(const char *model) {
    ...
    if (pid == 0) {
        ...
        execl(NPU_SERVER, NPU_SERVER, model, NULL);
        _exit(127);
    }
